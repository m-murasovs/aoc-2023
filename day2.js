...
const gamesLines = input.split('\n')

const gamesObjects = gamesLines.map((game) => {
    const [gameId, results] = game.split(':');

    // array of arrays of strings => array of objects
    const resultsObjectsArr = results
        .split(';')
        .map(result => {
            // array of strings => object
            const score = result.trim().split(',');
            const gameObj = {}

            score.forEach((set) => {
                const [count, color] = set.trim().split(' ');
                gameObj[color] = parseInt(count)
            })

            return gameObj;
        })
    return {
        [gameId]: resultsObjectsArr,
    }
})
...
// For each game, ge the maximum number of each color cubes
// Map over the array and compare the highest number for each color to some state
// Save the highest value
const highestValues = gamesObjects.map((gameObj) => {
    const allScoresPerColor = {
        green: [],
        blue: [],
        red: [],
    }

    Object.values(gameObj).forEach(resultsArr => {
        // in each results array, gather all the red, green, and blue counts in arrays, then find the largest of each
        resultsArr.forEach(resultObj => {
            for (const [color, count] of Object.entries(resultObj)) {
                allScoresPerColor[color].push(count)
            }
        })
    ...
    const maxVals = {
        green: Math.max(...allScoresPerColor.green),
        blue: Math.max(...allScoresPerColor.blue),
        red: Math.max(...allScoresPerColor.red),
    }

    return maxVals.green * maxVals.blue * maxVals.red;
})

const sumPowers = highestValues.reduce((acc, curr) => acc + curr)
console.log(sumPowers)




// Get the sum of possible IDs
...
const possibleGames = gamesObjects
    .map((gameObj) => {
        // iterate all the sub-games in the object
        for (const [key, resultsArr] of Object.entries(gameObj)) {

            // Check if each sub-game could be possible
            const allSubgameBools = resultsArr.map((result) => {
                const isSubgamePossibe = Object.keys(result).every((color) => {
                    return result[color] <= possibleResults[color]
                })

                return isSubgamePossibe;
            })

            if (!allSubgameBools.includes(false)) return key;
        }
    })
    .filter(Boolean)

const allScores = possibleGames.map(game => parseInt(game.trim().split(' ')[1]), 10)
const sum = allScores.reduce((acc, curr) => acc + curr)
