...
/** 
p2
go through all the * and see if they have numbers adjacent
*/
...
const starsInLines = inputLines.map((line, index) => {
    const matches = getStarsInString(line)

    if (matches) {
        const matchesWithIndexes = matches.map(match => {
            return {
                index: match.index,
            }
        })
...
starsInLines.forEach(line => {
    const { lineNr, matches } = line;
    const currLine = inputLines[lineNr]
    const prevLine = inputLines[lineNr - 1]
    const nextLine = inputLines[lineNr + 1]

    matches.forEach(match => {
        const { index } = match;
        const allAdjacentNumbers = []
        /** 
        get numbers that are within the range of the star
        multiply the numbers
        only get numbers if there are TWO of them
        push all adjacent numbers to array and only multiply if there's two
        */
        const currLinePrevChar = currLine[index - 1] && !!currLine[index - 1].match(/[0-9]/);
        const currLineNextChar = currLine[index + 1] && !!currLine[index + 1].match(/[0-9]/);

        // get substring of 4 chars before index and extract number from it
        if (currLinePrevChar) {
            const prevSubstr = currLine.substring(index - 4, index);
            const number = prevSubstr.match(/[0-9]{1,}/);
            if (number) allAdjacentNumbers.push(parseInt(number[0], 10))
        }
        if (currLineNextChar) {
            const nextSubstr = currLine.substring(index, index + 4);
            const number = nextSubstr.match(/[0-9]{1,}/);
            if (number) allAdjacentNumbers.push(parseInt(number[0], 10))
        }

        if (prevLine) {
            const allPrevLineNumbers = getNumbersInString(prevLine)
            allPrevLineNumbers.forEach(numberMatch => {
                const endIndex = numberMatch.index + numberMatch[0].length;
                const isWithinRange = numberMatch.index === index || numberMatch.index === index + 1 || numberMatch.index === index - 1 || endIndex === index || endIndex === index + 1;
                if (isWithinRange) allAdjacentNumbers.push(parseInt(numberMatch[0], 10))
            })
        }

        if (nextLine) {
            const allNextLineNumbers = getNumbersInString(nextLine)
            allNextLineNumbers.forEach(numberMatch => {
                const endIndex = numberMatch.index + numberMatch[0].length;
                const isWithinRange = numberMatch.index === index || numberMatch.index === index + 1 || numberMatch.index === index - 1 || endIndex === index || endIndex === index + 1;
                if (isWithinRange) allAdjacentNumbers.push(parseInt(numberMatch[0], 10))
            })
        ...
        if (allAdjacentNumbers.length === 2) {
            gearRatios.push(allAdjacentNumbers[0] * allAdjacentNumbers[1])
        }
    });
})

// console.log(gearRatios)
console.log('sum', gearRatios.reduce((acc, curr) => acc + curr))
...
// Part 1
const numbersInLines = inputLines.map((line, index) => {
    // const reg = /\d{1,3}/gm;
    // const matches = line.match(reg);
    const matches = getNumbersInString(line)
...
const numbersDirectlyAdjacentToSymbol = [];
const skippedNumbers = [];

numbersInLines.forEach(line => {
    const { lineNr, matches } = line;
    const currLine = inputLines[lineNr]
    const prevLine = inputLines[lineNr - 1]
    const nextLine = inputLines[lineNr + 1]
    ...
    matches.forEach(match => {
        const { number, startIndex, lastIndex } = match;

        // Get numbers from current line
        const currLinePrevChar = currLine[startIndex - 1] && currLine[startIndex - 1].replace(/[a-zA-Z0-9.]/g, '');
        const currLineNextChar = currLine[lastIndex + 1] && currLine[lastIndex + 1].replace(/[a-zA-Z0-9.]/g, '');

        let hasPrevLineSymbolsWithinRange;
        // Get matches from prev line
        if (prevLine) {
            hasPrevLineSymbolsWithinRange = prevLine.substring(startIndex - 1, lastIndex + 2).replace(/[a-zA-Z0-9.]/g, '');
        }

        let hasNextLineSymbolsWithinRange;
        if (nextLine) {
            hasNextLineSymbolsWithinRange = nextLine.substring(startIndex - 1, lastIndex + 2).replace(/[a-zA-Z0-9.]/g, '');
        }
        ...
        //     currLinePrevChar,
        //     currLineNextChar,
        //     hasPrevLineSymbolsWithinRange,
        //     hasNextLineSymbolsWithinRange
        // })
        ...
        if (
            ...
            numbersDirectlyAdjacentToSymbol.push(parseInt(number, 10));
        } else {
...
// console.log(skippedNumbers, '327656', '552907')
// console.log('total: ', numbersDirectlyAdjacentToSymbol.reduce((acc, curr) => acc + curr))
